...
def generate_horizon_events(
        ...
        names,  # Either a str or a tuple of str
        starting_time=None,
        days: int=1,
        kind: tuple=RISE_SET,
        altitude: int=0):
...
def upcoming_rise_and_set_events():
    ...
    for event in generate_horizon_events(where, names, days=1):
        events.append(event)
    for event in generate_horizon_events(where, 'sun', days=1, kind=TWILIGHT, altitude=-6):
        events.append(event)
